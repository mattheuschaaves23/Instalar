...
export default function Opportunities() {
  ...
  const [eligibility, setEligibility] = useState({ can_express_interest: true });
  ...
  const loadOpportunities = useCallback(async (nextFilter = filter) => {
    ...
    try {
      ...
      setEligibility(response.data?.eligibility || { can_express_interest: true });
  ...
  return (
    <div className="page-shell opportunities-page">
      ...
      {!loading && eligibility.message ? (
        <div className="opportunity-access-note">
          <div>
      ...
      {!loading && accessRequirement ? (
        <div className="opportunity-empty">
          <strong>
            {accessRequirement.code === 'CERTIFICATION_REQUIRED'
              ? 'Seu perfil está aguardando verificação.'
              : accessRequirement.code === 'PROFILE_LOCATION_REQUIRED'
                ? 'Informe sua região de atendimento.'
                : 'Publique seu perfil profissional.'}
          </strong>
          <span>{accessRequirement.message}</span>
          <Link className="gold-button" to="/profile">Revisar meu perfil</Link>
        </div>
      ) : null}
              ...
              <article className="opportunity-row" key={opportunity.id}>
                ...
                <div className="opportunity-actions">
                  ...
                  {opportunity.selected_by_me && opportunity.whatsapp_url ? (
                    ...
                    eligibility.can_express_interest ? (
                      <button
